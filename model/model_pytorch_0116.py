...
class CRNN_MRI(nn.Module):
    ...
    def forward(self, x, k, m, test=False,model_save_dir=False,save_last=False):
        ...
        '''
        CRNN_MRI-forward-x: torch.float32
        CRNN_MRI-forward-x: torch.Size([1, 2, 192, 192, 18])
        CRNN_MRI-forward-k: torch.Size([1, 2, 192, 192, 18])
        CRNN_MRI-forward-m: torch.Size([1, 2, 192, 192, 18])
        '''
        ...
        for i in range(1,self.nc+1):
            ...
            # CRNN_MRI-x-1:shape torch.Size([72, 2, 192, 192])
            # CRNN_MRI-net-t-x4: torch.Size([72, 2, 192, 192])
            # print('CRNN_MRI-x-1:shape',x.shape)
            # print("CRNN_MRI-net-t-x4:",net['t%d_x4'%i].shape)
            net['t%d_out'%i] = x + net['t%d_x4'%i]

            net['t%d_out'%i] = net['t%d_out'%i].view(-1,n_batch, n_ch, width, height)
            # CRNN_MRI-net-t-out: torch.Size([18, 4, 2, 192, 192])
            # print("CRNN_MRI-net-t-out:",net['t%d_out'%i].shape)
            ...
            net['t%d_out'%i] = self.dcs[i-1].perform(net['t%d_out'%i], k, m, model_save_dir,save_last=save_last)            
            x = net['t%d_out'%i]
