...
# nohup python train_kgin_base_vista_r4_zzy.py --config config_kgin_base_vista_r4_zzy.yaml > log_kgin_base_vista_r4_0524.txt 2>&1 &
...
class TrainerAbstract:
    def __init__(self, config):
        ...
        if self.debug: config.general.exp_name = 'test_kgin_vista_r4'
...
class TrainerKInterpolator(TrainerAbstract):
    ...
    def run_test(self):
        ...
        with torch.no_grad():
            ...
            # 打印结果
            print(f'\nkgin_base_vista_r4 Validation PSNR - Mean: {psnr_mean:.4f} ± {np.sqrt(psnr_var):.4f} | Variance: {psnr_var:.4f}')
            ...
            try:
                np.save('out_kgin_vista_r4_0524.npy', out)
            except OSError as e:
