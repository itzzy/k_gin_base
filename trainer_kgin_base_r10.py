...
# nohup python train_kgin_base_r10.py --config config_kgin_base_r10.yaml > log_0107_test.txt 2>&1 &
...
class TrainerAbstract:
    def __init__(self, config):
        ...
        if self.debug: config.general.exp_name = 'test_kgin_r10'
...
class TrainerKInterpolator(TrainerAbstract):
    ...
    def run_test(self):
        ...
        with torch.no_grad():
            ...
            # 打印结果
            print(f'\nkgin_base_vista_r10 Validation PSNR - Mean: {psnr_mean:.4f} ± {np.sqrt(psnr_var):.4f} | Variance: {psnr_var:.4f}')
            ...
            try:
                np.save('out_kgin_base_0319_r10.npy', out)
            except OSError as e:
