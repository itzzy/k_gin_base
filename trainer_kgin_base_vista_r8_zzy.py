...
# nohup python train_kgin_base_vista_r8_zzy.py --config config_kgin_base_vista_r8_zzy.yaml > log_0216_test.txt 2>&1 &
...
class TrainerAbstract:
    def __init__(self, config):
        ...
        # if self.debug: config.general.exp_name = 'test_kgin_vista_r8_zzy'
        # test_kgin_shear_grid_r8_zzy
        if self.debug: config.general.exp_name = 'test_kgin_shear_grid_r8_zzy'
...
class TrainerKInterpolator(TrainerAbstract):
    ...
    def run_test(self):
        ...
        with torch.no_grad():
            ...
            try:
                np.save('out_kgin_shear_grid_r8_zzy_0312.npy', out)
            except OSError as e:
