...
import os
from medutils.visualization import center_crop # 中心裁剪图像
from scipy.interpolate import interp1d # 插值处理
from matplotlib import gridspec # 复杂子图布局
import matplotlib.ticker as mticker # 坐标轴格式化
...
def visualize_results(config_path):
    ...
    # Crop ref_data and comp_data_list to the same size of the last one # 中心裁剪到相同尺寸
    ...
    # Plot comparison results # 绘制比较数据
    for idx, comp_data in enumerate(comp_data_list):
        ...
        # Plot ROI area # # 绘制ROI、误差图、X-T剖面等
    ...
    # plt.show()
    # plt.savefig('comparison.pdf', dpi=300, bbox_inches='tight') # 保存图像
