...
class TrainerAbstract:
    def __init__(self, config):
        ...
        self.nc = config.general.nc
        # 通过索引取出列表中的元素（因为这里只有一个元素）并转换为整数类型
        self.nc = int(self.nc[0])
    ...
    def run_test(self,epoch):
        ...
        with torch.no_grad():
            ...
            for i, (kspace, coilmaps, sampling_mask) in enumerate(self.test_loader):
                ...
                # print('train_one_epoch-im_recon-test-loss:',im_recon.requires_grad)
                running_test_loss += img_test_loss.item()
                # torch.cuda.empty_cache()
                # 计算 PSNR
                ...
                if i % 10 == 0:
                    ...
                    # vis.append((
                    #     from_tensor_format(im_gnd[0]),
                    #     # from_tensor_format(im_recon.data.cpu().numpy())[0],
                    #     from_tensor_format(img['t%d' % (self.nc-1)].data.cpu().numpy())[0],
                    #     from_tensor_format(im_und[0]),
                    #     from_tensor_format(mask.data.cpu().numpy(), mask=True)[0]
                    # ))
            ...
            print(" test PSNR max:\t\t{:.6f}".format(np.max(epoch_psnr)))
...
def prep_input(im, mask, acc=4.0):
    ...
    """
    # print('prep_input-im-shape:',im.shape) #torch.Size([1, 18, 192, 192])
    # print('prep_input-mask-shape:',mask.shape) #torch.Size([1, 18, 192, 192])
